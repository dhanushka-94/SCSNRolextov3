        ...
        <div class="grid w-full max-w-5xl overflow-hidden rounded-3xl border border-sand bg-paper shadow-[0_24px_60px_rgba(27,77,50,0.12)] lg:grid-cols-[1.05fr_0.95fr]">
            ...
            <div class="px-6 py-10 sm:px-10 lg:px-12">
                <div class="mb-8 lg:hidden">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.18em] text-leaf">Planter Portal</p>
                ...
                <h2 class="hidden text-2xl font-semibold text-forest-dark lg:block">Planter Portal</h2>
                <p class="mt-1 hidden text-sm text-muted lg:block">Sign in with your planter account.</p>

                @if (session('success'))
                    <div class="mt-5 rounded-xl border border-leaf/20 bg-leaf/10 px-4 py-3 text-sm text-forest">
                        {{ session('success') }}
                    </div>
                @endif
                ...
                <form method="POST" action="{{ route('planter.login.store') }}" class="mt-8 space-y-5">
                    @csrf

                    <div>
                        <label for="login" class="label-field">Email</label>
                        <input id="login" name="login" type="text" value="{{ old('login') }}" required autofocus autocomplete="username" class="input-field" placeholder="you@email.com">
                    </div>
                ...
                <p class="mt-6 text-center text-sm text-muted">
                    New planter?
                    <a href="{{ route('planter.register') }}" class="font-semibold text-leaf hover:text-forest">Register now</a>
                </p>
                <p class="mt-2 text-center text-sm text-muted">
                    Approved, no password yet?
